...
import { LayoutDashboard, Video, BarChart3, Settings, LogOut, Sparkles, Menu, X, ChevronLeft } from 'lucide-react';
...
const navItems = [
  ...
  { to: '/videos', icon: Video, label: 'Meus Jogos' },
...
const AppLayout = () => {
    ...
    <>
      ...
      <nav className={`flex-1 transition-all duration-300 ${sidebarExpanded ? 'px-2 space-y-1' : 'px-1 space-y-2 flex flex-col items-center'}`}>
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 rounded-lg transition-colors shrink-0 ${
      ...
      <div className={`border-t border-border transition-all duration-300 flex flex-col items-center ${sidebarExpanded ? 'p-4' : 'p-3'}`}>
        {planType === 'free' && sidebarExpanded && (
          <button onClick={() => setPricingOpen(true)} className="w-full mb-3 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-primary/10 text-primary text-sm font-medium hover:bg-primary/20 transition-colors">
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex bg-background">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static z-50 lg:z-auto top-0 left-0 h-full ${sidebarExpanded ? 'w-64' : 'w-20'} bg-card border-r border-border flex flex-col transition-all duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidebarContent />
      </aside>
