 ...
 * Direito de retificação (LGPD Art. 18, III)
 */
router.patch(
  ...
  [
    ...
    body('avatarUrl').optional().isURL({ require_protocol: true }).withMessage('Invalid avatar URL'),
    body('role').optional().isIn(['athlete', 'coach', 'analyst']).withMessage('Invalid role'),
  ],
  validate,
  gdprController.rectifyProfileData
);
